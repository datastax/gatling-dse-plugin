...
import com.datastax.driver.dse.graph.{ContinuousPagingGraphResult, GraphProtocol, GraphResultSet, GraphStatement}
...
class GraphResponseHandler(val next: Action,
  ...
  override protected def newResponse(rs: GraphResultSet): GraphResponse = new GraphResponse(JavaConverters.iterableAsScalaIterable(rs), rs.getExecutionInfo, dseAttributes)
  override protected def queriedHost(rs: GraphResultSet): String = rs.getExecutionInfo.getQueriedHost.toString
}

class ContinuousGraphResponseHandler(
                                      val next: Action,
                                      val session: Session,
                                      val system: ActorSystem,
                                      val statsEngine: StatsEngine,
                                      ...
                                      val dseAttributes: DseGraphAttributes,
                                      val metricsLogger: MetricsLogger)
  extends DseResponseHandler[ContinuousPagingGraphResult, GraphResponse] {
  ...
  override protected def newResponse(rs: ContinuousPagingGraphResult): GraphResponse = new GraphResponse(JavaConverters.iterableAsScalaIterable(rs), rs.getExecutionInfo, dseAttributes)

  override protected def queriedHost(rs: ContinuousPagingGraphResult): String = rs.getExecutionInfo.getQueriedHost.toString
}
