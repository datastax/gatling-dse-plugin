...
object ResultSetUtils {
  ...
  def asyncResultSetToIterator(resultSet:AsyncResultSet): Iterator[Row] =
    new AsyncResultSetIterator(
      new CqlAsyncRS(resultSet))

  def asyncGraphResultSetToIterator(resultSet:AsyncGraphResultSet): Iterator[GraphNode] =
    new AsyncResultSetIterator(
      new GraphAsyncRS(resultSet))
}

object GraphResultSetUtils {

  private def buildFilterAndMapFn[T]
  (filterFn: GraphNode => Boolean, mapFn: GraphNode => T)
  (resultSet: AsyncGraphResultSet, key:String):Iterator[T] = {
    ResultSetUtils.asyncGraphResultSetToIterator(resultSet )
...
class AsyncResultSetIterator[T](rs:AsyncRS[T]) extends Iterator[T] {
  var working = rs
